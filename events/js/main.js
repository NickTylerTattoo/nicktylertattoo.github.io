...
const FAQ = [
  ...
  ['How far ahead do we need to book?', "Three to four weeks at the least, and that is the permit, not me. Suffolk County requires a temporary body art permit for the event, and the county needs three to four weeks to process it once your date and venue are locked. So the soonest date I can honestly hold is about a month out. Anything further ahead is easy, and the 2027 season is already booking."],
  ...
  ['Where do you travel?', "Long Island, the Hamptons, and New York City. Suffolk County events need the date locked three to four weeks out for county permits, so the earlier you reach out, the better."],
...
if (!STILL) {
  ...
  /* This page drives a scroll-scrubbed canvas and three videos off the GSAP
     ticker. Backgrounded, that is pure wasted GPU work — and enough of it to
     take a browser's GPU process down. Park everything while hidden. */
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      gsap.ticker.sleep();
      $$('video').forEach(v => { if (!v.paused) { v.dataset.wasPlaying = '1'; v.pause(); } });
    } else {
      gsap.ticker.wake();
      $$('video').forEach(v => {
        if (v.dataset.wasPlaying === '1') { delete v.dataset.wasPlaying; v.play().catch(() => {}); }
      });
    }
  }, { passive: true });
}
